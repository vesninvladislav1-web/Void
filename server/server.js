...
// Перенос аккаунта ссылкой. Срок больше, чем у входа с компьютера: там
// человек стоит у двух экранов сразу, а тут ссылку надо ещё куда-то
// переслать и открыть. Но не настолько больше, чтобы забытая ссылка
// годилась через час.
const TTL_ПЕРЕНОСА  = 5 * 60000;
const МАКС_ПЕРЕНОСОВ = 500;
...
const входы  = new Map();
// Переносы аккаунта ссылкой: опознаватель -> { создан, пакет }
// Внутри пакета — шифротекст, ключ от которого живёт ТОЛЬКО в самой ссылке,
// в её хвосте после #. Браузер такой хвост на сервер не отправляет, поэтому
// сервер физически не может прочитать, что переносят, даже захотев.
const переносы = new Map();
...
const server = http.createServer((req, res) => {
  ...
  // ===== ПЕРЕНОС АККАУНТА ССЫЛКОЙ =====
  // Телефон кладёт сюда зашифрованный свёрток со своей личностью и получает
  // опознаватель. Ключ от свёртка сюда не приходит никогда: он живёт в
  // хвосте ссылки после #, а такой хвост браузер на сервер не шлёт.
  // Поэтому «сервер не может прочитать перенос» — не обещание, а устройство.
  if (req.method === 'POST' && url.pathname === '/api/perenos/new') {
    if (!rateLimit('perenosnew:' + clientIp(req), 20, 600000)) {
      return json(429, { error: 'Слишком часто, подожди' });
    }
    readBody(async (body) => {
      const u = await verifyUser(body.nick, body.password);
      if (!u) return json(401, { error: 'Неверный ник или пароль' });
      if (u.banned) return json(403, { error: 'Аккаунт заблокирован' });
      const пакет = typeof body.пакет === 'string' ? body.пакет : '';
      if (!пакет || пакет.length > MAX_KEY_BODY) return json(400, { error: 'Неверные данные' });
      if (переносы.size >= МАКС_ПЕРЕНОСОВ) return json(503, { error: 'Сервер занят, попробуй позже' });
      const ид = crypto.randomBytes(16).toString('hex');
      переносы.set(ид, { создан: Date.now(), пакет });
      console.log('[перенос] Свёрток принят для', u.nick);
      json(200, { ok: true, id: ид, срок: Math.round(TTL_ПЕРЕНОСА / 1000) });
    }, MAX_KEY_BODY);
    return;
  }

  // Отдаём ровно один раз и тут же забываем: ссылка одноразовая, и если её
  // уже открыли — второй раз она не сработает ни у кого, включая того, кто
  // подсмотрел её в переписке.
  if (req.method === 'POST' && url.pathname === '/api/perenos/take') {
    if (!rateLimit('perenostake:' + clientIp(req), 60, 600000)) {
      return json(429, { error: 'Слишком часто, подожди' });
    }
    readBody(async (body) => {
      const ид = typeof body.id === 'string' ? body.id : '';
      const п = переносы.get(ид);
      // Срок проверяем здесь, а не только в общей уборке: уборка ходит раз в
      // несколько минут, и без этой строки просроченная ссылка успела бы
      // сработать в промежутке между её обходами.
      if (!п || Date.now() - п.создан > TTL_ПЕРЕНОСА) {
        переносы.delete(ид);
        return json(404, { error: 'Ссылка недействительна или уже использована' });
      }
      переносы.delete(ид);
      console.log('[перенос] Свёрток забран');
      json(200, { ok: true, пакет: п.пакет });
    });
    return;
  }
...
function погаситьВходы() {
  ...
  let снятоП = 0;
  for (const [ид, п] of переносы) if (now - п.создан > TTL_ПЕРЕНОСА) { переносы.delete(ид); снятоП++; }
  if (снятоП) console.log('[перенос] Просрочено свёртков:', снятоП);
}
