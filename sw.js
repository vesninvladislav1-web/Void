...
// Шрифты кладём в кэш вместе со страницей — тогда без интернета Void
// выглядит как задуман, а не запасным системным шрифтом.
//
// Здесь ТОЛЬКО те наборы символов, которыми набран сам интерфейс: кириллица
// и латиница. Греческий, вьетнамский и расширенные наборы лежат на сервере
// и подтянутся, если в чьём-то сообщении встретится такая буква, — но
// заранее тащить их в кэш всем незачем, это лишние двести килобайт.
const ШРИФТЫ = [
  './shrifty/shrifty.css',
  './shrifty/inter-cyrillic.woff2',
  './shrifty/inter-latin.woff2',
  './shrifty/space-mono-400-latin.woff2',
  './shrifty/space-mono-700-latin.woff2'
];
const ASSETS = ['./', './index.html', './manifest.json', './icon.svg', './icon-192.png', './icon-512.png', ...ШРИФТЫ];
...
self.addEventListener('push', e => {
  ...
  e.waitUntil((async () => {
    ...
    // Кончается место на сервере. Приходит только владельцу, и он должен
    // понять причину, не открывая приложение: тут дорога каждая минута.
    if (data.disk) {
      const гб = data.gb || '?';
      await self.registration.showNotification('На сервере кончается место', {
        body: 'Свободно ' + гб + ' ГБ. Вложения скоро перестанут приниматься, переписка продолжит работать.',
        icon: './icon-192.png',
        badge: './icon-192.png',
        tag: 'void-disk',
        requireInteraction: true,
    ...
    // Срочная жалоба. Приходит только владельцу и только про то, что
    // сортировщик счёл опасным: поток уведомлений про каждую ссору читать
    // перестанут через день, и тогда пропустят важное.
    if (data.zhaloba) {
      await self.registration.showNotification('Жалоба, которая ждёт', {
        body: 'Похоже на: ' + (data.category || 'неизвестно') + '. Открой админ-панель → Жалобы.',
        icon: './icon-192.png',
        badge: './icon-192.png',
        tag: 'void-zhaloba',
        requireInteraction: true,
